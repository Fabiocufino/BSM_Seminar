...
subtitle: Higgs mechanism and upper bound on the Higgs mass
...
The potentfial has two diffenet forms:
...
## Consequences of SB in the Abelian Model {auto-animate=true}
### Consequences of Symmetry Breaking

- *Perturbation theroy*: It is convenient to rewrite $\phi$ 
...
$$
  \mathcal{L} = -\frac{1}{4} F_{\mu\nu}^2 + \frac{1}{2} e^2 v^2 A_\mu^2 + \cdots
$$
</div>
...
<div style="font-size: smaller;">
[Gauge field $A_\mu$ acquires a mass]{.fg}: $\;\; m_A = ev$
</div>
...
::: {.column width="45%"}
<div style="font-size: smaller;">
- Terms in the potential with $h_\mu$:
</div>
<div style="font-size:0.8em; text-align:center; margin-bottom: 20px;">
$$
...
<div style="font-size: smaller;">
- The Goldstone boson $\chi(x) \;\; \Longrightarrow \;\;$  massles scalar field
</div>
...
$$ 
  ...
  - Goldstone boson has been 'eaten' to give the photon mass. 
...
- Movement around the flat direction of the potential.
...
- $SU(2)_L$ and $U(1)_Y$ coupling constants are $g$ and $g'$

## Scalar Potential and VEV in Weinberg-Salam Model
### Scalar Potential and VEV

- Coupled to the gause fields the complex scalar $SU(2)$ doublet $\Phi = \begin{pmatrix} \phi^+ \\ \phi^0 \end{pmatrix}$
...
- $SU(2)$ local Gauge Transformation:
  - Higgs mechanism $\phi(x)$ ($SU(2)$ doublet): $\;\; \phi' = \left( I + ig_W \epsilon(x) \cdot \mathbf{T} \right)\phi$
  - $LH$ doublet of fermion fields: $\;\;  \bar{L}' = \bar{L}\left(I - ig_W \epsilon(x) \cdot \mathbf{T} \right)$


## Higgs and Fermions
### Leptons and up-type quarks masses 
...
- Achieved by constructing the conjugate doublet $\;\; \phi_c \equiv i \tau_2 \phi^* = \begin{pmatrix} \phi^0 \\ -\phi^- \end{pmatrix}$
  - A gauge invariant mass term can be constructed from $(\bar{L}\phi_c R + \bar{R}\phi^{\dagger}_c L)$

$$
   \mathcal{L}_u = -\frac{g_u v}{\sqrt{2}} (\bar{u}_L u_R + \bar{u}_R u_L) - \frac{g_u}{\sqrt{2}} h (\bar{u}_L u_R + \bar{u}_R u_L)
$$
